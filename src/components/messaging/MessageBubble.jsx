  ...
  return (
    <div className={`group flex items-end ${isMine ? "justify-end" : "justify-start"} ${rowMargin}`}>
      ...
      <div className="flex items-end gap-0.5">
        ...
        <div
          onContextMenu={(e) => onMenu(msg, e)}
          className={`relative max-w-[85%] sm:max-w-[72%] lg:max-w-[62%] px-3.5 py-2 text-[14px] leading-relaxed animate-fade-in ${bubbleRadius(
            isMine,
            groupStyle
          )} ${bubbleBg} ${bubbleBorder} ${bubbleShadow} ${isMine ? "text-secondary-100 dark:text-secondary-100" : dark ? "text-secondary-100" : "text-secondary-800"}`}
        >
          {showQuote && <QuotedBlock replyTo={msg.reply_to} quoteName={quoteName} dark={dark} mine={isMine} />}
          <p className={`whitespace-pre-wrap break-words ${showTime ? "pr-10" : "pr-1"}`}>{msg.content}</p>
          {showTime && (
            <div
              className={`absolute bottom-[3px] right-2.5 flex items-center gap-1 ${
